(function() {
  ...
  */
  ...
  /*
    Once the map loads, load _all_ of the layers at 0 opacity
  */
  ...
  */
  ...
  function getUserDetail(){
    if (lookupActiveLayerFromZoom(map.getZoom())=='z15'){
      ...
      var users = {}
      getUniqueFeatures(features, 'quadkey').forEach(function(tile){
        JSON.parse(tile.properties.users).forEach(function(user){
          if (users.hasOwnProperty(user.name)){
            users[user.name].editCount += Number(user.editCount)
            users[user.name].newFeatureCount += Number(user.newFeatureCount)
          }else{
            users[user.name] = user
          }
        })
      })
      _.sortBy(users, function(u){return -u.editCount}).forEach(function(user){
        console.log(user)
        $("#user-details tbody").append(`<tr><td>${user.name}</td><td>${user.editCount}</td><td>${user.newFeatureCount}</td></tr>`);
      })
    }
  }

  $('#load-user-detail').on('click',function(){
    getUserDetail()
  })
